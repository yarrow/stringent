...
fn test_killed_by_signal() {
    if let Some(status) = killed_by_signal() {
        let result = Ok(status).verify();
        match result {
            Err(CommandStatusError::Signal(Some(XCPU))) => {}
...
#[test]
fn bad_arguments_with_output() {
    let cmd = "/bin/sleep";
    if Path::new(cmd).is_file() {
        match Command::new(cmd).output().verify() {
            Ok(_) => panic!("{} should not have succeeded", cmd),
            Err(e) => match e.err {
                CommandStatusError::ExitCode(_) => match e.output {
                    Some(stringent::Std { stdout: _, stderr }) if stderr.len() > 0 => {}
                    _ => panic!("Expected to capture {}'s stderr", cmd),
                },
                _ => panic!("Unexpected error ({}) in executing {}", e.err, cmd),
            },
        }
    }
}
